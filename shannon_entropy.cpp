...
// Returns the Shannon entropy of a distribution as a float [0, 1]
// 0 indicates the distribution is as far from uniform as possoble
// 1 indicates a perfectly uniform distribution
double shannon_entropy(const std::vector<int>& dist)
{
	...
	for (int y_i: dist)
	{
		double p_i = double(y_i) / sum;
		entropy += -p_i * std::log2(p_i);
	}
	return entropy / std::log2(dist.size());
}
...
int main()
{
	...
	for (int i = 0; i < infiles.size(); ++i)
	{
		...
		for (int k = 1; k <= 24; ++k)
		{
			std::cout << shannon_entropy(substring_distribution(k)) << std::endl;
			//outfile << k << ", ";
	...
	read_file_into_array("./generators/c_stdlib_rand/randomness.txt");
